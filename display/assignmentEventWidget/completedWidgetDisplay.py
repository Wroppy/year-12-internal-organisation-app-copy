...
class CompletedWidgetDisplay(QWidget):
    def __init__(self):
        super(CompletedWidgetDisplay, self).__init__()
        layout = QVBoxLayout(self)
        layout.setSpacing(0)

        # Creates the header
        header = Header("Completed Assignments")
        layout.addWidget(header)

        # Creates the widget holder
        self.widgetHolder = WidgetHolder([Assignment("Assignment", True) for _ in range(10)])
...
if __name__ == '__main__':
    app = QApplication()
    display = CompletedWidgetDisplay()
    display.show()
    app.exec()
